  ...
  providers: [
    ...
    FacebookProvider({
      ...
      async profile(profile) {
        // Here we handle the profile received from Facebook
        // Generate a username based on profile information if needed
        ...
        if (!user) {
          user = await db.user.create({
            data: {
              username,
              email: profile.email,
              password: '', // You can choose to hash a default password if needed
              createdAt: new Date(),
              updatedAt: new Date(),
            },
          });
